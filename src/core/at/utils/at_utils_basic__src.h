...
void at_utils__cfg_init(at_utils_t *utils, zcom_cfg_t *cfg, const char *data_dir, at_bool_t verbose)
{
  ...
  if (zcom_cfg__get(cfg, &utils->temp_thermostat, "T0", "%lf") != 0) {
    if (verbose) fprintf(stderr, "at-info: assuming default utils->temp_thermostat = 300.0, key: T0\n");
  }
  ...
  at_utils_trace__cfg_init(utils->trace, cfg, utils->ssm, data_dir, verbose);
