    ...
    const char *fn)
{
  ...
  if (zcom_endn__fread(&itmp, sizeof(itmp), 1, io->fp, io->endn) != 1) {
    fprintf(stderr, "Error@%s: error in reading sizeof(double)\n",
        io->module);
    goto ERR;
  }
  if (itmp != (int) sizeof(double)) {
    fprintf(stderr, "Error@%s: (int) sizeof(double) mismatch, expect: %d, read: %d, pos: %s,%#lx\n",
        io->module, (int) sizeof(double), itmp, io->fn, (unsigned long) ftell(io->fp));
    goto ERR;
  }

  io->version = 0;
  if (zcom_endn__fread(&io->version, sizeof(int), 1, io->fp, io->endn) != 1) {
    fprintf(stderr, "Error@%s: error in reading version, fn %s\n",
        io->module, io->fn);
    goto ERR;
  }
