...
int at_load_data(at_t *at, at_bool_t bCPT)
{
  ...
  read_mb = bCPT;
  if (read_mb) {
     /* read previous mb_t data */
    if (mb__read(mb, at->langevin) != 0) {
      fprintf(stderr, "cannot load mb data from %s\n", mb->av_file);
      return 1;
    }

    mb__write_ze_file(mb, "ze_init.dat");
          ...
          at_bool_t bfirst, at_bool_t blast, at_bool_t btr, at_bool_t bflush)
{
  ...
  if (at_do_every(step, at->mb->nst_save_av, bfirst, blast)) { /* save averages */
    ...
    mb__write_ze_file(at->mb, NULL);
    if (at->mtrng) {
      mtrng_save(at->mtrng, at->rng_file);
    }
  }
...
int at_cfg_init(at_t *at, cfg_t *cfg, double boltz, double tmstep)
{
  ...
  exit_if((at == NULL), "null pointer to at_t\n");

  exit_if((cfg == NULL), "cannot load *.cfg file\n");

  /* bmin: minimal beta (highest temperature) */
  exit_if(cfg_get(cfg, &at->bmin, "beta_min", "%lf") != 0,
      "missing var: at->bmin, key: beta_min, fmt: %%lf\n");

  exit_if((at->bmin <= 0.0), "at->bmin should be positive!\n");

  /* bmax: maximum beta (lowest temperature) */
  exit_if(cfg_get(cfg, &at->bmax, "beta_max", "%lf") != 0,
      "missing var: at->bmax, key: beta_min, fmt: %%lf\n");

  exit_if((at->bmax <= 0.0), "at->bmax: should be positive!\n");

  exit_if(!(at->bmax > at->bmin), "at->bmax: failed validation: at->bmax > at->bmin\n");

  /* T0: thermostat temperature */
  at->T0 = 300.0;
  if (cfg_get(cfg, &at->T0, "T0", "%lf")) {
    fprintf(stderr, "assuming default: at->T0 = 300.0, key: T0\n");
  }

  /* nsttemp: frequency of tempering, 0: disable, -1: only ns */
  at->nsttemp = -1;
  if (cfg_get(cfg, &at->nsttemp, "nsttemp", "%d")) {
    fprintf(stderr, "assuming default: at->nsttemp = -1, key: nsttemp\n");
  }

  /* mvreps: number of repeating Langevin eq */
  at->mvreps = 1;
  if (cfg_get(cfg, &at->mvreps, "move_repeats", "%d")) {
  ...
  at->nst_log = -1;
  if (cfg_get(cfg, &at->nst_log, "nsttrace", "%d")) {
...

  {
    ...
    if (cfg_get(cfg, &fn_rng, "rng_file", "%s")) {
      fprintf(stderr, "assuming default: at->rng_file = \"%s\", key: rng_file\n", fn_rng);
    }

    at->rng_file = utils_make_output_filename(at->ssm, at->data_dir, fn_rng);
  }

  {
    ...
    if (cfg_get(cfg, &fn_log, "log_file", "%s"))
    {
      fprintf(stderr, "assuming default: at->log_file = \"%s\", key: log_file\n", fn_log);
    }

    at->log_file = utils_make_output_filename(at->ssm, at->data_dir, fn_log);
  ...
  at->bTH = 0;
  if (cfg_get(cfg, &at->bTH, "boost_mode", "%d"))
  {
  ...
  at->TH_Tref = 300.0;
  if (at->bTH) {
    if (cfg_get(cfg, &at->TH_Tref, "boost_Tref", "%lf"))
    {
  ...
  at->kappa0 = 1.0;
  if (at->bTH) {
    if (cfg_get(cfg, &at->kappa0, "kappa0", "%lf")) {
  ...
  at->epsilon0 = 0.0;
  if (at->bTH)
    if (cfg_get(cfg, &at->epsilon0, "epsilon0", "%lf"))
      fprintf(stderr, "assuming default: at->epsilon0 = 0.0, key: epsilon0\n");

  int silent = 0;

  /* mb: handle for multiple-bin estimator */
  mb__cfg_init(at->mb, cfg, boltz, at->bmin, at->bmax, at->data_dir, silent);

  langevin_cfg_init(at->langevin, cfg, silent);

  /* eh: energy histogram */
  eh__cfg_init(at->eh, cfg, at->mb, at->data_dir);
...
int at_manifest(at_t *at, const char *fn, int arrmax)
{
  ...
  if (at->mb != NULL) {
    ...
    mb__manifest(at->mb, fp, arrmax);
  }

  if (at->eh != NULL) {
    fprintf(fp, "at->eh: object pointer to eh_t\n");
    eh__manifest(at->eh, fp, arrmax);
  }

  if (at->langevin != NULL) {
    fprintf(fp, "at->langevin: object pointer to langevin_t\n");
    langevin_manifest(at->langevin, fp, arrmax);
  }
