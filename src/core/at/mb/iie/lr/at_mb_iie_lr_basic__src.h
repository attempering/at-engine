...
void at_mb_iie_lr__cfg_init(at_mb_iie_lr_t *lr, at_mb_iie_zerofiller_t *zf, at_mb_t *mb, zcom_cfg_t *cfg, at_bool_t verbose)
{
  ...
  lr->frac_min = 0.0;
  if (0 != zcom_cfg__get(cfg, &lr->frac_min, "mbest-fracmin", "%lf")) {
    if (verbose) fprintf(stderr, "Info@at: assuming default mb->iie->lr->frac_min = 0.0, key: mbest-fracmin\n");
  }
  ...
  lr->cv_shift_max = 1.0;
  if (0 != zcom_cfg__get(cfg, &lr->cv_shift_max, "mbest-cvshiftmax", "%lf")) {
    if (verbose) fprintf(stderr, "Info@at: assuming default mb->iie->lr->cv_shift_max = 1.0, key: mbest-cvshiftmax\n");
  }
