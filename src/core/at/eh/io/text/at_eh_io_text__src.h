...
static int at_eh__read_text_low_level(at_eh_t *eh, at_utils_io_t *io)
{
  ...
  if (at_utils_io_text__read_and_compare_int(io, NULL, "eh->cnt", eh->cnt) != 0) {
    goto ERR;
  }

  /* eh_min: minimal energy */
  if (at_utils_io_text__read_and_compare_double(io, NULL, "eh->min", eh->min, 1e-5) != 0) {
    goto ERR;
  }

  /* eh_del: energy bin size */
  if (at_utils_io_text__read_and_compare_double(io, NULL, "eh->del", eh->del, 1e-5) != 0) {
    goto ERR;
  }
  ...
  for (i = 0; i < eh->n; i++) {
    ...
    if (jmin >= eh->cnt) {
      fprintf(stderr, "Error@at.eh.io.text: eh->his: base index %d out of boundary [0, eh->cnt=%d)\n",
          jmin, eh->cnt);
...
static int at_eh__write_text_low_level(at_eh_t *eh, at_utils_io_t *io)
{
  ...
  if (at_utils_io_text__write_int(io, eh->cnt, "eh->cnt") != 0) {
    goto ERR;
  }

  if (at_utils_io_text__write_double(io, eh->min, "eh->min") != 0) {
    goto ERR;
  }

  if (at_utils_io_text__write_double(io, eh->del, "eh->del") != 0) {
    goto ERR;
  }
