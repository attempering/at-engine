...
#ifndef AT_EH_IO_TEXT__SRC_H__
#define AT_EH_IO_TEXT__SRC_H__
...
static int at_eh__read_text_low_level(at_eh_t *eh, at_utils_io_t *io)
{
  ...
  if (at_utils_io_text__read_and_compare_int(io, NULL, "eh->n", eh->n) != 0) {
    goto ERR;
  }

  /* eh_cnt: number of energy bins */
  if (at_utils_io_text__read_and_compare_int(io, NULL, "eh->e_n", eh->e_n) != 0) {
    goto ERR;
  }

  /* eh_min: minimal energy */
  if (at_utils_io_text__read_and_compare_double(io, NULL, "eh->e_min", eh->e_min, 1e-5) != 0) {
    goto ERR;
  }

  /* eh_del: energy bin size */
  if (at_utils_io_text__read_and_compare_double(io, NULL, "eh->e_del", eh->e_del, 1e-5) != 0) {
    goto ERR;
  }
  ...
  for (i = 0; i < eh->n; i++) {
    ...
    if (at_utils_io_text__read_and_compare_int(io, NULL, "eh:index", i) != 0) {
      goto ERR;
    }

    if (at_utils_io_text__read_int(io, &jmin, "eh:jmin",
        AT_UTILS_IO__NONNEGATIVE) != 0) {
      goto ERR;
    }

    if (jmin >= eh->e_n) {
      at_utils_logger__error(eh->logger, "eh->his: base index %d out of boundary [0, eh->e_n=%d)\n",
          jmin, eh->e_n);
      goto ERR;
    }

    if (at_utils_io_text__read_int(io, &size, "eh:size",
        AT_UTILS_IO__NONNEGATIVE) != 0) {
      goto ERR;
    }
    ...
    if (jmin + size > eh->e_n) {
      at_utils_logger__error(eh->logger, "%s: invalid size %d, jmin=%d, [0, eh->e_n=%d)\n",
          io->fn, size, jmin, eh->e_n);
      goto ERR;
    }

    if (at_utils_io_text__read_double_array(io, size,
...
int at_eh__read_text(at_eh_t *eh, const char *fn, int *version)
{
  ...
  if (at_eh__read_text_low_level(eh, io) != 0) {
    goto ERR;
  }
...
static int at_eh__write_text_low_level(at_eh_t *eh, at_utils_io_t *io)
{
  int i;
  ...
  if (at_utils_io_text__write_int(io, eh->e_n, "eh->e_n") != 0) {
    goto ERR;
  }

  if (at_utils_io_text__write_double(io, eh->e_min, "eh->e_min") != 0) {
    goto ERR;
  }

  if (at_utils_io_text__write_double(io, eh->e_del, "eh->e_del") != 0) {
    goto ERR;
  }
  ...
  for (i = 0; i < eh->n; i++) {
    ...
    if (at_utils_io_text__write_int(io, i, "eh:index") != 0) {
      goto ERR;
    }

    if (at_utils_io_text__write_int(io, jmin, "eh:jmin") != 0) {
      goto ERR;
    }
...
int at_eh__write_text(at_eh_t *eh, const char *fn, int version)
{
  ...
  if (at_eh__write_text_low_level(eh, io) != 0) {
    goto ERR;
  }
