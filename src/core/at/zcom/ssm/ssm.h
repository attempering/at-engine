...
typedef struct ss_llist_node_t_ {
  size_t size;
  char *s;
  struct ss_llist_node_t_ *next;
} ss_llist_node_t;

typedef ss_llist_node_t ss_llist_t;


typedef struct ssm_t_ {
  int hash_bits;
  size_t hash_size;
  size_t block_size; /* minimum number of bytes */
  ss_llist_t *base_; /* base_[0..bash_size-1]*/
} ssm_t;
